...
def get_subject_mask_v2(img_rgba):
    """改进的抠图算法"""
    ...
    cx, cy = w // 2, h // 2
    cv2.ellipse(center_mask, (cx, cy), (w//3, h//2), 0, 0, 360, 255, -1)
    mask[center_mask > 0] = cv2.GC_FGD
    
    rect = (30, 30, w - 60, h - 60)
    cv2.grabCut(img_bgr, mask, rect, bgd, fgd, 8, cv2.GC_INIT_WITH_MASK)
    
    # 提取前景
    result = np.where((mask == cv2.GC_FGD) | (mask == cv2.GC_PR_FGD), 255, 0).astype("uint8")
    
    # 与肤色蒙版合并
    result = cv2.bitwise_or(result, skin_mask)
    ...
    if contours:
        ...
        cv2.drawContours(result, [max_contour], -1, 255, -1)
...
def main():
    if len(sys.argv) < 2:
        print("用法: python3 generate_layers_v2.py 图片路径 [顶部文字] [底部文字]")
        sys.exit(1)
    ...
    print("抠图中 (v2: 肤色检测 + GrabCut)...")
    subject_mask = get_subject_mask_v2(card)
